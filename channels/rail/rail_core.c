	...
	)
{
	...
	DEBUG_RAIL("rail_core_handle_server_hadshake: session=0x%p buildNumber=0x%X.",
	...
	rail_vchannel_send_client_information_order(session,
    ...
    )
{
	...
	DEBUG_RAIL("rail_core_handle_server_minmax_info: session=0x%p "
		...
		max_pos_x, max_pos_y,
		min_track_width, min_track_height,
		...
		)
{
	...
	DEBUG_RAIL("rail_core_handle_server_get_app_resp: session=0x%p "
		"window_id=0x%X app_id=(length=%d dump>)",
		session, window_id, app_id->length
		);
#ifdef WITH_DEBUG_RAIL
		...
		)
{
	...
	if (sys_param.type == SPI_SETHIGHCONTRAST)
	{
		...
		rail_string2unicode_string(session, &color_scheme,
				&sys_param.value.high_contrast_system_info.color_scheme);
	}
...
//------------------------------------------------------------------------------
static void
rail_core_handle_ui_execute_remote_app(
		RAIL_SESSION* session,
		RAIL_UI_EVENT * event
		)
{
		...
		)
{
	rail_vchannel_send_activate_order(
		session,
		event->param.activate_info.window_id,
		((event->param.activate_info.enabled == True) ? 1 : 0)
		);
}
//------------------------------------------------------------------------------
static void
rail_core_handle_ui_sys_command(
		RAIL_SESSION* session,
		RAIL_UI_EVENT * event
		)
{
...
//------------------------------------------------------------------------------
static void
rail_core_handle_ui_window_move(
		RAIL_SESSION* session,
		RAIL_UI_EVENT * event
		)
{
...
//------------------------------------------------------------------------------
static void
rail_core_handle_ui_system_menu(
		RAIL_SESSION* session,
		RAIL_UI_EVENT * event
		)
{
...
//------------------------------------------------------------------------------
static void
rail_core_handle_ui_get_app_id(
		RAIL_SESSION* session,
		RAIL_UI_EVENT * event
		)
{
	rail_vchannel_send_get_appid_req_order(
		session,
		event->param.get_app_id_info.window_id
		);
}
//------------------------------------------------------------------------------
void
